...
def run_add_weight(args):
    ...
    md = load_metadata(args)
    merged_dir = os.path.join(args.outputdir, 'merged')
    status_file = os.path.join(merged_dir, '.success')
...
#        return
    ...
    if args.use_tmpdir:
        ...
        tmp_merged_dir = os.path.join(tmpdir, os.path.basename(args.outputdir), 'merged')
        logging.info('Using tmpdir %s for merging.' % tmp_merged_dir)
        if not os.path.exists(tmp_merged_dir):
            os.makedirs(tmp_merged_dir)

    merged_files = []
    failed_merges = []

    for samp in md['samples']:
        outfile = os.path.join(tmp_merged_dir if args.use_tmpdir else merged_dir, f'{samp}_tree.root')

        # Skip if the output file already exists
        if os.path.exists(outfile):
            logging.info(f'Merged file {outfile} already found, skipping merging for {samp}.')
            continue

        cmd = f'haddnano.py {outfile} {args.outputdir}/pieces/{samp}_*_tree.root'
        logging.debug('...' + cmd)
        ...
        while attempts < 3:
            ...
            if 'error' in log_lower or 'fail' in log_lower:
                logging.error(log)
                failed_merges.append(samp)
            if p.returncode == 0:
                break
            attempts += 1

        if attempts == 3 and p.returncode != 0:
            logging.error(f'Failed merging {samp} after 3 attempts.')
            failed_merges.append(samp)
        else:
...
#        if args.weight_file:
        if False:
            try:
                ...
                if '-' not in samp and '_' not in samp:
                    # Data, no weight to add
                    logging.info(f'Not adding weight to sample {samp}')
                else:
                    raise KeyError(f'No xsec found for sample {samp}')

        if args.use_tmpdir:
            shutil.copy(outfile, merged_dir)
            os.remove(outfile)

    # Print summary
    if merged_files:
        logging.info('Successfully merged files:')
        for merged_file in merged_files:
            logging.info(merged_file)

    if failed_merges:
        logging.info('Failed to merge files:')
        for failed_merge in failed_merges:
            logging.info(failed_merge)
...
#     )
    ...
    parser.add_argument(
        '--add-weight', action='store_true', default=True,
...
def run(args, configs=None):
    ...
    host = socket.getfqdn()
    if 'cern.ch' in host:
        args.use_tmpdir = False

    if args.post:
        args.add_weight = False
        args.merge = True
